...
**Pull requests** are welcome for fixes and translations of label defaults. Anything that changes `city.json` field names or structure is a network-level decision (the schema is a shared contract across all city sites) — propose it in an issue first; final call sits with the Foundation (luc@fab.city).
